...
import { signUpForJobs, confirmJobsSignup, resendJobsCode } from "@/app/jobs/signup/actions";
import { TurnstileWidget } from "@/components/reviews/TurnstileWidget";

// Shaped like src/components/bizup/landing/SignupForm.tsx: same two-step
// code flow, same "use a different email" escape hatch. Four fields, same
// reasoning as KatisoBiz's own -- every field here costs a job seeker who
// has already spent minutes building a CV, so nothing is asked that is not
// needed to log back in later.
...
export function SignupForm() {
  const [state, action, pending] = useActionState(signUpForJobs, null);
  const [confirmState, confirmAction, confirming] = useActionState(confirmJobsSignup, null);
  const [resendState, resendAction, resending] = useActionState(resendJobsCode, null);
  ...
  return (
    ...
    >
      <label className={label}>
        Your full name
        <input name="fullName" autoComplete="name" placeholder="Sipho Ndlovu" className={input} />
        {state?.error?.fullName?.[0] && <span className={err}>{state.error.fullName[0]}</span>}
      </label>
      ...
      <label className={label}>
        Your email
        <input name="email" type="email" inputMode="email" autoComplete="email" placeholder="you@example.com" className={input} />
        <span className="text-xs font-normal text-neutral-500">
          We send a code here to check it works. It is how you log back in.
        </span>
      ...
      <button
        ...
        className="mt-1 inline-flex w-full items-center justify-center rounded-full bg-neutral-900 px-6 py-3.5 text-sm font-semibold text-white shadow-sm transition hover:-translate-y-0.5 hover:bg-neutral-800 disabled:opacity-50"
      >
        {pending ? "Sending your code..." : "Save my CV"}
      </button>
