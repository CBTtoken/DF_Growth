...
type EventDetail = {
  ...
  contact_details: { email: string; phone: string | null; whatsapp: string | null } | null;
  images: string[] | null;
...
async function getEvent(id: string): Promise<EventDetail | null> {
  ...
  const { data } = await admin
    .from("events")
    .select(
      "id, event_name, description, start_datetime, end_datetime, location_address, city, event_type, social_links, contact_details, images, ticket_info_text"
    )
    .eq("id", id)
    .eq("status", "published")
    .maybeSingle();
    ...
    <main className="flex flex-1 flex-col bg-gray-50">
      ...
      {heroUrl ? (
        <div className="relative h-64 w-full overflow-hidden bg-gray-100 sm:h-80">
          <Image src={heroUrl} alt={event.event_name} fill sizes="100vw" className="object-cover" priority />
        </div>
      ) : (
        <div className="grid h-40 w-full place-items-center bg-brand/5 text-5xl" aria-hidden>
          📅
        </div>
      )}
      ...
      <section className="mx-auto w-full max-w-3xl flex-1 px-4 py-10 sm:px-6">
        <div className="flex flex-col gap-6 rounded-2xl border border-gray-100 bg-white p-6 shadow-sm sm:p-10">
          <div className="flex flex-col gap-3">
            <span className="w-fit rounded-full bg-brand/10 px-2.5 py-0.5 text-xs font-semibold uppercase text-brand">
              {typeLabel}
            </span>
            <h1 className="text-2xl font-bold tracking-tight text-ink sm:text-3xl">{event.event_name}</h1>
