...
**Public Events section** (`/events`) — search, city filter, event-type filter, sorted soonest first. Past events automatically drop out of the browse view once their date (or end date, for multi-day events) passes, retained in the database rather than deleted. A daily cron job (the shared `/api/cron/daily` runner, see Section 24) formally transitions past published events to `status = 'expired'` — purely data tidiness, since the browse page's own query filter already hides them regardless.
...
- **Automated backups** — weekly full database dump via GitHub Actions (stays on GitHub because it needs the `pg_dump` binary), independent of Supabase's own plan tier.
- **Automated cron jobs** — the three daily jobs (expire past events, onboarding nudges, trial reminders) run via **Vercel Cron** as a single consolidated `/api/cron/daily` endpoint that invokes all three in sequence (each independently, one failing never skips the others). Moved off GitHub Actions on 2026-07-22: the old GitHub workflows curled the public site over the internet and intermittently failed at the TLS handshake (Vercel's platform bot-mitigation resetting GitHub's shared runner IPs), sending a false-alarm failure email on ~1 run in 4. Vercel Cron triggers from inside Vercel's own network, eliminating that hop. Vercel auto-attaches `Authorization: Bearer $CRON_SECRET` (set in Production), which each job validates. Only the weekly DB backup remains a GitHub Actions cron.
...
- **Automation:** Vercel Cron for the three daily jobs (trial-expiry reminders, onboarding nudges, expire past events) via one consolidated `/api/cron/daily` endpoint; GitHub Actions for the weekly database backup and the Legacy Reactivation staged batch-send
